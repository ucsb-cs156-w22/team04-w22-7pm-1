...
describe('LoadSubjectsPage tests', () => {
  ...
  test('test what happens when you reload and retrieve', async () => {
    const queryClient = new QueryClient();
    axiosMock
      .onGet('api/admin/UCSBSubjects/all')
      .replyOnce(200, [])
  ...
  test('test what happens when you click delete a subject in a non-empty list', async () => {
    const queryClient = new QueryClient();
    axiosMock
      .onGet('/api/admin/UCSBSubjects/all')
      .replyOnce(200, loadsubjectsFixtures.fourSubjects)
      .onGet('/api/admin/UCSBSubjects/all')
      .reply(200, loadsubjectsFixtures.threeSubjects);
    axiosMock
      .onPost('/api/admin/UCSBSubjects/load')
    ...
    await waitFor(() => {
      expect(mockToast).toBeCalledWith('1 subject was deleted');
    });
  });

  test('test what happens when you click delete multiple subjects in a non-empty list', async () => {
    const queryClient = new QueryClient();
    axiosMock
      .onGet('/api/admin/UCSBSubjects/all')
      .replyOnce(200, loadsubjectsFixtures.fourSubjects)
      .onGet('/api/admin/UCSBSubjects/all')
      .reply(200, loadsubjectsFixtures.oneSubject);
    axiosMock
      .onPost('/api/admin/UCSBSubjects/load')
      .replyOnce(200, loadsubjectsFixtures.fourSubjects)
      .onPost('/api/admin/UCSBSubjects/load')
    ...
    await waitFor(() => {
      expect(mockToast).toBeCalledWith('3 subjects were deleted');
    });
  });
});
